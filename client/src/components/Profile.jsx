...
const Profile = () => {
  ...
  const trackModalRef = useRef(null);
  ...
  // Handle Filtering when clicking a car or track
  const handleCarFilter = async (carId) => {
    const userId = sessionStorage.getItem('userId');
    if (selectedCarFilter === carId) {
      // Toggle off if already selected
      setSelectedCarFilter('');
      setSelectedTrackFilter('');
      setFilteredRuns(profile.runs);
      return;
    }
    setSelectedCarFilter(carId);
    setSelectedTrackFilter(''); // Clear track filter when selecting car
    try {
      const res = await api.get(`/runs/user/${userId}?carId=${carId}`);
      setFilteredRuns(res.data);
    } catch (err) {
      console.error("Failed to filter runs by car", err);
    }
  };

  const handleTrackFilter = async (trackId) => {
    const userId = sessionStorage.getItem('userId');
    if (selectedTrackFilter === trackId) {
      // Toggle off if already selected
      setSelectedTrackFilter('');
      setSelectedCarFilter('');
      setFilteredRuns(profile.runs);
    ...
    setSelectedCarFilter(''); // Clear car filter when selecting track
    try {
      const res = await api.get(`/runs/user/${userId}?trackId=${trackId}`);
      setFilteredRuns(res.data);
    } catch (err) {
      console.error("Failed to filter runs by track", err);
    }
  };
  ...
  // --- REGEX VALIDATIONS ---
  const textPattern = /^[a-zA-Z0-9\s\-]+$/; // Letters, numbers, spaces, hyphens
  const yearPattern = /^(19|20)\d{2}$/;     // Years 1900-2099
  const numberPattern = /^[1-9]\d*$/;        // Positive numbers greater than 0

  if (!textPattern.test(vehicleForm.make) || !textPattern.test(vehicleForm.model)) {
    return alert("Марката и модела содржат невалидни карактери!");
  }
  if (!yearPattern.test(vehicleForm.year)) {
    return alert("Внесете валидна година (на пр. 1990 - 2026)!");
  }
  if (!numberPattern.test(vehicleForm.engineDisplacement) || !numberPattern.test(vehicleForm.hp)) {
  ...
  try {
    ...
    if (vehicleModalRef.current) {
      const modal = Modal.getInstance(vehicleModalRef.current) || new Modal(vehicleModalRef.current);
      modal.hide();
    }
  ...
  // --- REGEX VALIDATIONS ---
  const nameLocationPattern = /^[a-zA-ZÀ-ÿ0-9\s\-\.]+$/; // Letters, numbers, spaces, dots, hyphens
  const lengthPattern = /^\d+(\.\d{1,2})?/;              // Positive numbers with up to 2 decimals

  if (!nameLocationPattern.test(trackForm.name) || !nameLocationPattern.test(trackForm.location)) {
    return alert("Името на патеката или локацијата содржи невалидни специјални карактери!");
  }
  if (!lengthPattern.test(trackForm.length) || parseFloat(trackForm.length) <= 0) {
    return alert("Должината на патеката мора да биде валиден позитивен број!");
  }
  // -------------------------
  ...
  try {
    ...
    if (trackModalRef.current) {
      const modal = Modal.getInstance(trackModalRef.current) || new Modal(trackModalRef.current);
      modal.hide();
    }
    document.querySelectorAll('.modal-backdrop').forEach(el => el.remove());
  ...
  const deleteRun = async (id) => {
    ...
    try {
      ...
      setFilteredRuns(prev => prev.filter(r => r._id !== id));
    } catch (err) {
      alert("Грешка при бришење.");
    }
  };
  ...
  const { user, vehicles, tracks } = profile;
  ...
  return (
    <div className="container-fluid py-4 text-white">
        ...
        <div className="col-md-9">
          <div className="row">
            ...
            <div className="col-md-6">
              <h3>Гаража <small className="text-muted fs-6" style={{cursor: 'pointer'}}>(Кликни за филтер)</small></h3>
              <div id="garageList" className="d-flex flex-column gap-2 mb-3">
                {vehicles.length > 0 ? (
                  vehicles.map(v => (
                    <div 
                      key={v._id} 
                      onClick={() => handleCarFilter(v._id)}
                      className={`card text-white p-2 mb-2 d-flex flex-row align-items-center justify-content-between`}
                      style={{ 
                        cursor: 'pointer', 
                        backgroundColor: selectedCarFilter === v._id ? '#b8860b' : '#6c757d',
                        borderColor: selectedCarFilter === v._id ? '#ffc107' : 'transparent',
                        borderWidth: selectedCarFilter === v._id ? '2px' : '1px'
                      }}
                    >
                      <span>{v.make} {v.model} ({v.year}) - {(v.engineDisplacement || 0).toFixed(1)} L - {v.hp} HP [{v.carRank}]</span>
                      <button className="btn btn-sm btn-danger ms-auto" onClick={(e) => deleteVehicle(v._id, e)}>X</button>
                    </div>
                  ))
                ) : (
              ...
              <button 
                ...
                onClick={() => {
                  const modalInstance = Modal.getOrCreateInstance(vehicleModalRef.current);
                  modalInstance.show();
                }}
              >
                Креирај Возило
            ...
            {/* Tracks Section */}
            <div className="col-md-6">
              <h3>Твои Патеки <small className="text-muted fs-6" style={{cursor: 'pointer'}}>(Кликни за филтер)</small></h3>
              <div id="tracksList" className="d-flex flex-column gap-2 mb-3">
                {tracks.length > 0 ? (
                  tracks.map(t => (
                    <div 
                      key={t._id} 
                      onClick={() => handleTrackFilter(t._id)}
                      className={`card text-white p-2 mb-2 d-flex flex-row align-items-center justify-content-between`}
                      style={{ 
                        cursor: 'pointer', 
                        backgroundColor: selectedTrackFilter === t._id ? '#b8860b' : '#6c757d',
                        borderColor: selectedTrackFilter === t._id ? '#ffc107' : 'transparent',
                        borderWidth: selectedTrackFilter === t._id ? '2px' : '1px'
                      }}
                    >
                      <span>{t.name}</span>
                      <button className="btn btn-sm btn-danger ms-auto" onClick={(e) => deleteTrack(t._id, e)}>X</button>
                    </div>
                  ))
                ) : (
              ...
              <button 
                ...
                className="btn btn-primary" 
                onClick={() => {
                  const modalInstance = Modal.getOrCreateInstance(trackModalRef.current);
                  modalInstance.show();
                }}
              >
                Креирај Патека
      ...
      <div className="row mt-5">
        <div className="col-12">
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h3>Статистика (Графикон) {selectedCarFilter || selectedTrackFilter ? <span className="badge bg-warning text-dark fs-6">Филтрирано</span> : null}</h3>
            {(selectedCarFilter || selectedTrackFilter) && (
              <button className="btn btn-outline-warning btn-sm" onClick={clearFilters}>Исчисти филтер</button>
            )}
          </div>
          ...
          {filteredRuns.length > 0 ? (
            <>
              ...
              <table className="table table-dark table-hover">
                ...
                <tbody id="myRunsList">
                  {filteredRuns.runs ? null : filteredRuns.map(r => (
                    <tr key={r._id}>
            ...
            <p className="text-muted">Нема снимени вожњи за избраниот филтер.</p>
          )}
        </div>
      </div>

      {/* --- POPUP MODAL: Add Vehicle --- */}
      <div className="modal fade" ref={vehicleModalRef} id="vehicleModal" tabIndex="-1" aria-hidden="true">
      ...
      <div className="modal fade" ref={trackModalRef} id="trackModal" tabIndex="-1" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content bg-dark text-light border border-warning">
            <div className="modal-header border-warning">
